...
function pidByName(name) {
  ...
  const found = Object.entries(playerNames || {}).find(([, playerName]) =>
    String(playerName).toLowerCase() === target
  );
...
test('Fernando Mendoza remains ahead of Ty Simpson by rookie market order', () => {
  const mendoza = pidByName('Fernando Mendoza') || IDS.FERNANDO_MENDOZA;
  const simpson = pidByName('Ty Simpson') || IDS.TY_SIMPSON;
  if (!pScore(mendoza) || !pScore(simpson)) { skip('Mendoza/Simpson not in snapshot'); return; }
  assert(pMeta(mendoza).pos === 'QB' && pMeta(simpson).pos === 'QB', 'Both players should be QB rookies');
  assert((pMeta(mendoza).fcValue || 0) > (pMeta(simpson).fcValue || 0),
    `FantasyCalc order inverted: Mendoza ${pMeta(mendoza).fcValue} <= Simpson ${pMeta(simpson).fcValue}`);
  assert(pScore(mendoza) > pScore(simpson),
    `DHQ order inverted: Mendoza ${pScore(mendoza)} <= Simpson ${pScore(simpson)}`);
});

test('Cam Ward keeps young starting QB protection above the rookie-QB floor', () => {
  const cam = pidByName('Cam Ward') || IDS.CAM_WARD;
  if (!pScore(cam)) { skip('Cam Ward not in snapshot'); return; }
  assert(pMeta(cam).pos === 'QB', `Cam Ward position ${pMeta(cam).pos}`);
  assert(pScore(cam) >= 2500, `Cam Ward ${pScore(cam)} below young starting QB floor`);
  assert((pMeta(cam).fcValue || 0) >= 3000, `Cam Ward FC value ${pMeta(cam).fcValue} below starter neighborhood`);
});

test('RB neighborhood: Kaleb Johnson is below Kenneth Walker but not erased', () => {
  const walker = pidByName('Kenneth Walker') || IDS.KENNETH_WALKER;
  const kaleb = pidByName('Kaleb Johnson') || IDS.KALEB_JOHNSON;
  if (!pScore(walker) || !pScore(kaleb)) { skip('Walker/Kaleb not in snapshot'); return; }
  assert(pMeta(walker).pos === 'RB' && pMeta(kaleb).pos === 'RB', 'Both players should be RBs');
  assert(pScore(walker) > pScore(kaleb), `Walker ${pScore(walker)} <= Kaleb ${pScore(kaleb)}`);
  const ratio = pScore(kaleb) / Math.max(1, pScore(walker));
  assertRange(ratio, 0.20, 0.75, `Kaleb/Walker DHQ ratio`);
});

test('WR neighborhood: Josh Downs and Mike Evans stay comparable', () => {
  const evans = pidByName('Mike Evans') || IDS.MIKE_EVANS;
  const downs = pidByName('Josh Downs') || IDS.JOSH_DOWNS;
  if (!pScore(evans) || !pScore(downs)) { skip('Evans/Downs not in snapshot'); return; }
  assert(pMeta(evans).pos === 'WR' && pMeta(downs).pos === 'WR', 'Both players should be WRs');
  const ratio = pScore(downs) / Math.max(1, pScore(evans));
  assertRange(ratio, 0.50, 1.25, `Downs/Evans DHQ ratio`);
});
...
function calcAcceptance(myVal, theirVal, dnaKey, taxTotal = 0) {
  let likelihood = 50;
...
test('Fair trade with no taxes → neutral acceptance for every DNA', () => {
  const dnas = ['FLEECER', 'DOMINATOR', 'STALWART', 'ACCEPTOR', 'DESPERATE', 'NONE'];
  dnas.forEach(dna => {
    const pct = calcAcceptance(5000, 5000, dna);
    assert(pct === 50, `Fair trade (${dna}) expected 50%, got ${pct}%`);
  });
});

test('Underpaying 30%+ → all DNA types are low without tax help', () => {
  const dnas = ['FLEECER', 'DOMINATOR', 'STALWART', 'ACCEPTOR', 'DESPERATE', 'NONE'];
  dnas.forEach(dna => {
    const pct = calcAcceptance(3500, 5000, dna);
    assert(pct <= 10, `Underpay 30% vs ${dna}: ${pct}% (should be <= 10%)`);
  });
});

test('Overpaying 20% → raises acceptance for every DNA', () => {
  const dnas = ['FLEECER', 'DOMINATOR', 'STALWART', 'ACCEPTOR', 'DESPERATE', 'NONE'];
  dnas.forEach(dna => {
    const pct = calcAcceptance(6000, 5000, dna);
    assert(pct >= 80, `Overpay 20% vs ${dna}: ${pct}% (should be >= 80%)`);
  });
});

test('STALWART overpay helps instead of penalizing the offer', () => {
  const over = calcAcceptance(6500, 5000, 'STALWART');
  const fair = calcAcceptance(5000, 5000, 'STALWART');
  const under = calcAcceptance(3750, 5000, 'STALWART');
  assert(over > fair, `25%+ overpay vs STALWART should beat fair: over ${over}%, fair ${fair}%`);
  assert(fair > under, `Fair should beat underpay vs STALWART: fair ${fair}%, under ${under}%`);
});

test('Tax modifiers fold into effective surplus', () => {
  const acceptorTax = calcAcceptance(5000, 5000, 'ACCEPTOR', 10);
  const dominatorTax = calcAcceptance(5000, 5000, 'DOMINATOR', -40);
  assert(acceptorTax === 60, `+10 tax modifier expected 60%, got ${acceptorTax}%`);
  assert(dominatorTax === 10, `-40 tax modifier expected 10%, got ${dominatorTax}%`);
});
...
test('No acceptance ever below 5%', () => {
  const extreme = calcAcceptance(1000, 10000, 'FLEECER');
  assert(extreme >= 5, `Extreme underpay: ${extreme}%`);
});
