...
async function main() {
  ...
  const peakWindows = Object.fromEntries(Object.entries(ageCurveWindows).map(([pos, curve]) => [pos, curve.peak]));
  ...
  fc.forEach(d => {
    ...
    const curve = ageCurveWindows[pos] || { build:[22,24], peak:peakWindows[pos] || [23,29], decline:[29,31] };
    ...
    else if (age > peakEnd && age <= declineEnd) {
      const floor = {QB:0.78,RB:0.62,WR:0.68,TE:0.70,K:0.82,DL:0.70,LB:0.68,DB:0.66}[pos] || 0.68;
      const progress = (age - peakEnd) / Math.max(1, declineEnd - peakEnd);
      ageFactor = 1 - (1 - floor) * Math.max(0, Math.min(1, progress));
    }
    else if (age > declineEnd) ageFactor = Math.max(0.03, ({QB:0.78,RB:0.62,WR:0.68,TE:0.70,K:0.82,DL:0.70,LB:0.68,DB:0.66}[pos] || 0.68) - (age - declineEnd) * ({QB:0.12,RB:0.22,WR:0.18,TE:0.16,K:0.08,DL:0.15,LB:0.16,DB:0.18}[pos] || 0.16));
