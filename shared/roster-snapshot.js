...
(function () {
  ...
  function buildRosterSnapshot() {
    ...
    const valueWindowEnd = pos => window.App?.ageCurveWindows?.[pos]?.decline?.[1] || window.App?.peakWindows?.[pos]?.[1] || 29;

    // ── Aging risks (past position value window with significant DHQ) ──
    const agingRisks = allPlayerDHQs
      .filter(p => p.age && p.age > valueWindowEnd(p.position) && p.dhq >= 1000)
      .slice(0, 5);

    // ── Sell candidates (declining DHQ trajectory or past value window) ──
    const sellCandidates = (myRoster.players || [])
      .map(pid => {
        ...
        // Past value window = sell candidate
        if (peak && peak.label === 'Declining') return { name: nameFn(pid), position: pos, dhq, reason: 'Past value window — value declining' };
        if (age > valueWindowEnd(pos)) return { name: nameFn(pid), position: pos, dhq, reason: `${pos} past value window — production cliff approaching` };
        return null;
      })
      .filter(Boolean)
      .sort((a, b) => b.dhq - a.dhq)
