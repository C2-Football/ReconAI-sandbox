...
async function captureOwnerShifts(leagueId) {
  try {
    ...
    for (const [rid, profile] of Object.entries(profiles)) {
      ...
      if (diff >= 3) {
        const ownerName = getUser(window.S?.rosters?.find(r => String(r.roster_id) === String(rid))?.owner_id);
...
window.loadLeagueMemory = loadLeagueMemory;
window.captureRosterDiff = captureRosterDiff;
window.captureTradeOutcomes = captureTradeOutcomes;
window.captureOwnerShifts = captureOwnerShifts;
window.captureAIAccuracy = captureAIAccuracy;
window.captureUserPreferences = captureUserPreferences;
