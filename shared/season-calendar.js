...
(function () {
  ...
  // Parse a Sleeper draft timestamp. Sleeper stores these as unix timestamps
  // in ms. Fall back to null if unparseable.
  function _parseDraftTimestamp(raw) {
    if (!raw) return null;
  ...
  function _draftFromLeagueOrDrafts(league, season) {
    const metadata = league?.metadata || {};
    const drafts = (window.S && Array.isArray(window.S.drafts) ? window.S.drafts : null)
      || (Array.isArray(league?.drafts) ? league.drafts : []);

    let draftTs = metadata.draft_date;
    let draftType = metadata.draft_type;

    if (!draftTs && drafts.length) {
      const sameSeason = drafts.find(d =>
        String(d.season) === String(season) &&
        (d.start_time || d.scheduled_time || d.start_ts)
      );
    ...
    return { draftDate: _parseDraftTimestamp(draftTs), draftType: draftType || null };
  }
  ...
  function getKeyDates(leagueRef) {
    ...
    const { draftDate, draftType } = _draftFromLeagueOrDrafts(league, season);

    return { draftDate, draftType, tradeDeadline, playoffStart, seasonStart, championshipWeek };
  }
